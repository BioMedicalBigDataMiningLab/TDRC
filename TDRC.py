...
def TDRC(X, S_d,S_m, r=4, beta=0.25, gam=0.1, lam=0.1, alpha=0.1, tol=1e-7, max_iter=500):
    ...
    for i in range(max_iter):
        ...
        M_2 = CG(0, beta * O_1, O_1, beta * C.T * S_m * C, lam, 0.01, 200)

        M_3 = CG(0, gam * O_2, O_2, gam * P.T * S_d * P, lam, 0.01, 200)

        K = np.mat(np.eye(r))

        F = C * M_2
        J = (beta * S_m.T * F + rho_1 * C + Y_1) * np.linalg.inv(beta * F.T * F + rho_1 * np.eye(r))
        Q = M_2 * J.T
        C = (X_1 * G + beta * S_m * Q.T + rho_1 * J - Y_1) * np.linalg.inv(
            G.T * G + beta * Q * Q.T + alpha * K + rho_1 * np.eye(r))

        R = P * M_3
        W = (gam * S_d.T * R + rho_2 * P + Y_2) * np.linalg.inv(gam * R.T * R + rho_2 * np.eye(r))
        Y_1 = Y_1 + rho_1 * (C - J)
        rho_1 = rho_1 * 1.1
        ...
        Z = M_3 * W.T
        P = (X_2 * U + gam * S_d * Z.T + rho_2 * W - Y_2) * np.linalg.inv(U.T * U + gam * Z * Z.T + alpha * K + rho_2 * np.eye(r))
        Y_2 = Y_2 + rho_2 * (P - W)
        rho_2 = rho_2 * 1.1
